...
class NetVLADModelLF(models.BaseModel):
  ...
  def create_model(self,
    ...
    liyc = MultiscaleCnnLstmModel()
    with tf.variable_scope("liyc"):
        liyc_lstm = liyc.create_model(model_input, vocab_size, num_frames, 
                   l2_penalty=1e-8, **unused_params)
    ...
    activation = tf.concat([activation, liyc_lstm],1)
    gating_size = hidden1_size + 1024
    if gating:
        gating_weights = tf.get_variable("gating_weights_2",
          [gating_size, gating_size],
          initializer = tf.random_normal_initializer(stddev=1 / math.sqrt(gating_size)))
