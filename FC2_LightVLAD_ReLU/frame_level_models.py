...
class NetVLADModelLF(models.BaseModel):
  ...
  def create_model(self,
    ...
    vlad_drop = tf.nn.dropout(vlad, 0.7)
    h_fc1 = tf.nn.relu6(tf.matmul(vlad_drop, hidden1_weights) +  hidden1_biases)
    h_fc1_drop = tf.nn.dropout(h_fc1, 0.8)
    activation = tf.nn.relu6(tf.matmul(h_fc1_drop, hidden2_weights) +  hidden2_biases)
    # h_fc2_drop = tf.nn.dropout(h_fc1, 0.8)
